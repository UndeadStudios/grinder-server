...
public abstract class Combat<T extends Agent> extends HitController<T> implements DebugListener {
    ...
    public boolean negateAggression = true;
    ...
    public void onHitApplied(Hit hit) {
        ...
        actor.ifPlayer(player -> {
            ...
            if (damage > 0)
                player.getCombat().handleItemEffect(attacker, hit);
        });
        ...
        if (recoilDamage > 0) {
            ...
            recoilTarget.getCombat().queue(Damage.create(recoilDamage));
        }
    }
    ...
    public void attack(Agent target) {
        ...
        if (target instanceof Player) {
            if (EntityExtKt.getBoolean(target, Attribute.STALL_HITS, false))
                return;
    ...
    public boolean isInCombat() {
        return isInActiveCombat() || !cooledDownFromCombat();
    }
    ...
    public boolean isInActiveCombat() {
        return isBeingAttacked() || (hasTarget() && isAttacking(target));
    }
    ...
    public boolean isMeleeAttack() {
        return getOrDetermineStrategy().type() == AttackType.MELEE;
    }

    public AttackStrategy<Agent> getOrDetermineStrategy() {
        return Optional.ofNullable(attackStrategy).orElse(determineStrategy());
    }
    ...
    boolean canBeReachedInCloseQuarter(final Agent attacker, int requiredDistance, final boolean triggerActions) {
        ...
        if (attacker.getMotion().isRunning()) {
            int extra = actor.getMotion().isRunning() ? 2 : actor.getMotion().isMoving() ? 1 : 0;
            requiredDistance += extra;
        } else if (attacker.getMotion().isMoving())
            requiredDistance += actor.getMotion().isMoving() || largeEntity ? 1 : 0;

        if (!LineOfSight.withinSight(attacker, actor, requiredDistance, attacker.getCombat().isMeleeAttack() && !attacker.getCombat().uses(HALBERD))) {
        ...
        if (targetDistance == 0 && AgentUtil.isFrozen(attacker)) {
            if (triggerActions)

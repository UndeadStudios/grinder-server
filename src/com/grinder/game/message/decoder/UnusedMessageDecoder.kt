package com.grinder.game.message.decoder

import com.grinder.game.message.MessageDecoder
import com.grinder.net.packet.Packet
import com.grinder.game.message.impl.UnusedMessage
import com.grinder.game.entity.agent.player.Player
import com.grinder.net.packet.PacketListener
import com.grinder.net.packet.PacketReader

/**
 * TODO: add documentation
 *
 * @author  Stan van der Bend (https://www.rune-server.ee/members/StanDev/)
 * @since   27/11/2019
 * @version 1.0
 */
class UnusedMessageDecoder: MessageDecoder<UnusedMessage>() {

	override fun decode(packet: Packet) : UnusedMessage {
		val reader = PacketReader(packet)
		return UnusedMessage()
	}
}

...
import com.grinder.game.cache.store.index.ChecksumTable;
import com.grinder.game.cache.store.index.Index;
...
public class CacheLibrary {
	 ...
	 */
	public Index addIndex(Compression.CompressionType compressionType, int version, int revision, boolean named, boolean whirlpool, boolean flag4, boolean flag8, boolean writeReferenceTabel) throws Exception {
		int id = indices.length;
		RandomAccessFile raf = new RandomAccessFile(new File(path, "main_file_cache.idx" + id), "rw");
		indices = Arrays.copyOf(indices, indices.length + 1);
		Index index = indices[id] = new Index(this, id, raf);
	...
	public Index addIndex(boolean named, boolean whirlpool) {
		try {
			...
			if (!checksumTable.writeArchiveSector(id,
					Compression.compress(outputStream.flip(), Compression.CompressionType.GZIP, null, -1))) {

...
public final class RegionRepository {
	...
	/**
	 * The map of Region Ids that correspond to the appropriate RegionSections. Region = 64x64
	 */
	private final Map<Integer, RegionSection> regionSection = new HashMap<>();
	...
	private void addSection(RegionSection section) {
		regionSection.put(section.getId(), section);
	}

	public void removeSection(int regionId) {
		regionSection.remove(regionId);
	}
	...
	public RegionSection getSection(int regionId) {
		RegionSection region = regionSection.get(regionId);
		if (region == null) {
			region = new RegionSection(regionId >> 8, regionId & 255);
			addSection(region);
		}
		return region;
	}
	...
	public void forRegion(int z, Boundary b, Consumer<Region> consumer) {
		...
		for (int x = 0, xL = endX - startX; x <= xL; x++) {
			for (int y = 0, yL = endY - startY; y <= yL; y++) {
				int chunkX = startX + x;
				int chunkY = startY + y;
				Region region = repo.get(new RegionCoordinates(chunkX, chunkY));

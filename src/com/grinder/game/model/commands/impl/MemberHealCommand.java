...
public class MemberHealCommand implements Command {
    ...
    public String getDescription() {
        return "Restores your prayer and hitpoints.";
    }

    @Override
    public void execute(Player player, String command, String[] parts) {
        if (!PlayerUtil.isAmethystMember(player)) {
            player.getPacketSender().sendMessage("<img=747> You must be an Amethyst member to use this command feature!");
            return;
        }
        ...
        if (player.getArea() != null && player.getArea() instanceof KalphiteLair && player.getPosition().getZ() == 0) { //This has the wrong z index
            player.getPacketSender().sendMessage("You can't use this command in this area.");
            return;
        }

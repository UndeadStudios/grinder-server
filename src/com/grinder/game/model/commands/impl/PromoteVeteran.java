...
public class PromoteVeteran implements Command {
	...
	public String getDescription() {
		return "Promotes/Demotes the player to Veteran rank.";
	}
    ...
    public void execute(Player player, String command, String[] parts) {
        ...
        if (EntityExtKt.getBoolean(plr.get(), Attribute.VETERAN, false)) {
        	player.getPacketSender().sendMessage("<img=779> You have demoted " + plr.get().getUsername() + " from the Veteran's Rank!");
        	plr.get().getPacketSender().sendMessage("<img=779> You have been demoted from the Veteran's rank by " + player.getUsername() + "!");
			plr.get().setTitle("");
			plr.get().updateAppearance();
            EntityExtKt.setBoolean(plr.get(), Attribute.VETERAN, false, false);
...
/*		if (plr.get().getGameMode().isIronman() || plr.get().getGameMode().isHardcore() || plr.get().getGameMode().isUltimate()) {
    	...
    	EntityExtKt.setBoolean(plr.get(), Attribute.VETERAN, true, true);
        plr.get().setCrown(PlayerRights.VETERAN.ordinal());
        plr.get().getPacketSender().sendRights();
        //plr.get().requestLogout();
        player.getPacketSender().sendMessage("" + player2 + " has been successfully promoted to Veteran's rank!");
        plr.get().getPacketSender().sendMessage("You have been promoted to Veteran's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"! Please relog for your status to show up.");
        PlayerUtil.broadcastMessage("<img=941> " + player2 + " has been promoted to Veteran's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"!");
    }

...
public class PromoteGlobalModerator implements Command {
    ...
    public String getDescription() {
        return "Promotes the player to Global Moderator rank.";
    }
...
/*        if (plr.get().getGameMode().isIronman() || plr.get().getGameMode().isHardcore() || plr.get().getGameMode().isUltimate()) {
        ...
        plr.get().setRights(PlayerRights.GLOBAL_MODERATOR);
        plr.get().setCrown(PlayerRights.GLOBAL_MODERATOR.ordinal());
        plr.get().getPacketSender().sendRights();
        //plr.get().requestLogout();
        player.getPacketSender().sendMessage("" + player2 + " has been successfully promoted to Global Moderator rank!");
        plr.get().getPacketSender().sendMessage("You have been promoted to Global Moderator rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"! Please relog for your status to show up.");
        PlayerUtil.broadcastMessage("<img=741> " + player2 + " has been promoted to Global Moderator rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"!");
        Logging.log("promotions", "" + player.getUsername() + " gave the global moderator rank to: " + plr.get().getUsername() + "");
    }

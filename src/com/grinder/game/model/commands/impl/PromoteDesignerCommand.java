...
public class PromoteDesignerCommand implements Command {
	...
	public String getDescription() {
		return "Promotes/Demotes the player to Designer rank.";
	}
    ...
    public void execute(Player player, String command, String[] parts) {
        ...
        if (EntityExtKt.getBoolean(plr.get(), Attribute.DESIGNER, false)) {
        	player.getPacketSender().sendMessage("<img=779> You have demoted " + plr.get().getUsername() + " from the Designer's Rank!");
        	plr.get().getPacketSender().sendMessage("<img=779> You have been demoted from the Designer's rank by " + player.getUsername() + "!");
			plr.get().setTitle("");
			plr.get().updateAppearance();
            EntityExtKt.setBoolean(plr.get(), Attribute.DESIGNER, false, false);
...
/*		if (plr.get().getGameMode().isIronman() || plr.get().getGameMode().isHardcore() || plr.get().getGameMode().isUltimate()) {
    	...
    	EntityExtKt.setBoolean(plr.get(), Attribute.DESIGNER, true, true);
        plr.get().setCrown(PlayerRights.DESIGNER.ordinal());
        plr.get().getPacketSender().sendRights();
        //plr.get().requestLogout();
        player.getPacketSender().sendMessage("" + player2 + " has been successfully promoted to Designer's rank!");
        plr.get().getPacketSender().sendMessage("You have been promoted to Designer's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"! Please relog for your status to show up.");
        PlayerUtil.broadcastMessage("<img=751> " + player2 + " has been promoted to Designer's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"!");
    }

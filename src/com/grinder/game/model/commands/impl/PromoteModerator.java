...
public class PromoteModerator implements Command {
	...
	public String getDescription() {
		return "Promotes the player to Moderator's rank.";
	}
...
/*		if (plr.get().getGameMode().isIronman() || plr.get().getGameMode().isHardcore() || plr.get().getGameMode().isUltimate()) {
        ...
        plr.get().setRights(PlayerRights.MODERATOR);
        plr.get().setCrown(PlayerRights.MODERATOR.ordinal());
        plr.get().getPacketSender().sendRights();
        //plr.get().requestLogout();
        player.getPacketSender().sendMessage("" + player2 + " has been successfully promoted to Moderator's rank!");
        plr.get().getPacketSender().sendMessage("You have been promoted to Moderator's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"! Please relog for your status to show up.");
        PlayerUtil.broadcastMessage("<img=740> " + player2 + " has been promoted to Moderator's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"!");
        Logging.log("promotions", "" + player.getUsername() + " gave the moderator's rank to: " + plr.get().getUsername() + "");
    }

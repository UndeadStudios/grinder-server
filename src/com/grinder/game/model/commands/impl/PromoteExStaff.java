...
public class PromoteExStaff implements Command {
	...
	public String getDescription() {
		return "Promotes/Demotes the player to Ex-Staff rank.";
	}
    ...
    public void execute(Player player, String command, String[] parts) {
        ...
        if (EntityExtKt.getBoolean(plr.get(), Attribute.EX_STAFF, false)) {
        	player.getPacketSender().sendMessage("<img=779> You have demoted " + plr.get().getUsername() + " from the Ex Staff Rank!");
        	plr.get().getPacketSender().sendMessage("<img=779> You have been demoted from the Ex Staff rank by " + player.getUsername() + "!");
			plr.get().setTitle("");
			plr.get().updateAppearance();
			EntityExtKt.setBoolean(plr.get(), Attribute.EX_STAFF, false, false);
...
/*		if (plr.get().getGameMode().isIronman() || plr.get().getGameMode().isHardcore() || plr.get().getGameMode().isUltimate()) {
		...
		EntityExtKt.setBoolean(plr.get(), Attribute.EX_STAFF, true, true);
		plr.get().setCrown(PlayerRights.EX_STAFF.ordinal());
        plr.get().getPacketSender().sendRights();
        //plr.get().requestLogout();
        player.getPacketSender().sendMessage("" + player2 + " has been successfully promoted to Ex staff rank!");
        plr.get().getPacketSender().sendMessage("You have been promoted to Ex staff rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"! Please relog for your status to show up.");
        PlayerUtil.broadcastMessage("<img=942> " + player2 + " has been given the Ex staff rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"!");
        Logging.log("promotions", "" + player.getUsername() + " gave the ex staff rank to: " + plr.get().getUsername() + "");
    }

    @Override
    public boolean canUse(Player player) {
    	return player.getUsername().equals("3lou 55") ||

...
public class TriggerFrogRandom implements Command {
	...
	public String getDescription() {
		return "Triggers a frog event for the player.";
	}
    ...
    public void execute(Player player, String command, String[] parts) {
    	...
    	// Trigger the event
		FrogPrinceEvent.INSTANCE.trigger(plr.get());

		// Action completed succesfully
		player.getPacketSender().sendMessage("<img=779> " + player2 + " has been successfully triggered to a frog random event!");
    ...
    public boolean canUse(Player player) {
    	return player.getUsername().equals("3lou 55") || player.getRights().equals(PlayerRights.DEVELOPER) ||
				player.getRights().equals(PlayerRights.ADMINISTRATOR) || player.getRights().equals(PlayerRights.GLOBAL_MODERATOR) ||

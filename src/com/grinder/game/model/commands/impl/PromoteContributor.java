...
public class PromoteContributor implements Command {
	...
	public String getDescription() {
		return "Promotes/Demotes the player to contributor's rank.";
	}
    ...
    public void execute(Player player, String command, String[] parts) {
        ...
        if (EntityExtKt.getBoolean(plr.get(), Attribute.CONTRIBUTOR, false)) {
        	player.getPacketSender().sendMessage("<img=779> You have demoted " + plr.get().getUsername() + " from the Contributor's Rank!");
        	plr.get().getPacketSender().sendMessage("<img=779> You have been demoted from the Contributor's rank by " + player.getUsername() + "!");
			plr.get().setTitle("");
			plr.get().updateAppearance();
			EntityExtKt.setBoolean(plr.get(), Attribute.CONTRIBUTOR, false, false);
...
/*		if (plr.get().getGameMode().isIronman() || plr.get().getGameMode().isHardcore() || plr.get().getGameMode().isUltimate()) {
		...
		EntityExtKt.setBoolean(plr.get(), Attribute.CONTRIBUTOR, true, true);
		plr.get().setCrown(PlayerRights.CONTRIBUTOR.ordinal());
        plr.get().getPacketSender().sendRights();
        //plr.get().requestLogout();
        player.getPacketSender().sendMessage("" + player2 + " has been successfully promoted to Contributor's rank!");
        plr.get().getPacketSender().sendMessage("You have been promoted to Contributor's rank by " + player.getUsername() + "! Please relog for your status to show up.");
        PlayerUtil.broadcastMessage("<img=1229> " + player2 + " has been promoted to Contributor's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"!");
		Logging.log("promotions", "" + player.getUsername() + " gave the contributor's rank to: " + plr.get().getUsername() + "");

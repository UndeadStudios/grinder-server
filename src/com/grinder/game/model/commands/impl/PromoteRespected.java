...
public class PromoteRespected implements Command {
	...
	public String getDescription() {
		return "Promotes/Demotes the player to Respected rank.";
	}
    ...
    public void execute(Player player, String command, String[] parts) {
        ...
        if (EntityExtKt.getBoolean(plr.get(), Attribute.RESPECTED, false)) {
        	player.getPacketSender().sendMessage("<img=779> You have demoted " + plr.get().getUsername() + " from the Respected Rank!");
        	plr.get().getPacketSender().sendMessage("<img=779> You have been demoted from the Respected rank by " + player.getUsername() + "!");
			plr.get().setTitle("");
			plr.get().updateAppearance();
			EntityExtKt.setBoolean(plr.get(), Attribute.RESPECTED, false, false);
...
/*		if (plr.get().getGameMode().isIronman() || plr.get().getGameMode().isHardcore() || plr.get().getGameMode().isUltimate()) {
		...
		EntityExtKt.setBoolean(plr.get(), Attribute.RESPECTED, true, true);
		plr.get().setCrown(PlayerRights.RESPECTED.ordinal());
        plr.get().getPacketSender().sendRights();
        //plr.get().requestLogout();
        player.getPacketSender().sendMessage("" + player2 + " has been successfully promoted to Respected rank!");
        plr.get().getPacketSender().sendMessage("You have been promoted to Respected rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"! Please relog for your status to show up.");
        PlayerUtil.broadcastMessage("<img=943> " + player2 + " has been promoted to Respected rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"!");
        Logging.log("promotions", "" + player.getUsername() + " gave the respected rank to: " + plr.get().getUsername() + "");
    }

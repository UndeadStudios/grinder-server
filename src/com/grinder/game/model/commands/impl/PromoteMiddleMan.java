...
public class PromoteMiddleMan implements Command {
	...
	public String getDescription() {
		return "Promotes/Demotes the player to Middleman rank.";
	}
    ...
    public void execute(Player player, String command, String[] parts) {
        ...
        if (EntityExtKt.getBoolean(plr.get(), Attribute.MIDDLEMAN, false)) {
        	player.getPacketSender().sendMessage("<img=779> You have demoted " + plr.get().getUsername() + " from the Middleman's Rank!");
        	plr.get().getPacketSender().sendMessage("<img=779> You have been demoted from the Middleman's rank by " + player.getUsername() + "!");
			plr.get().setTitle("");
			plr.get().updateAppearance();
			EntityExtKt.setBoolean(plr.get(), Attribute.MIDDLEMAN, false, false);
...
/*		if (plr.get().getGameMode().isIronman() || plr.get().getGameMode().isHardcore() || plr.get().getGameMode().isUltimate()) {
		...
		EntityExtKt.setBoolean(plr.get(), Attribute.MIDDLEMAN, true, false);
		plr.get().setCrown(PlayerRights.MIDDLEMAN.ordinal());
        plr.get().getPacketSender().sendRights();
        //plr.get().requestLogout();
        player.getPacketSender().sendMessage("" + player2 + " has been successfully promoted to Middleman's rank!");
        plr.get().getPacketSender().sendMessage("You have been promoted to Middleman's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"! Please relog for your status to show up.");
        PlayerUtil.broadcastMessage("<img=939> " + player2 + " has been promoted to trusted Middleman's rank by " + PlayerUtil.getImages(player) + "" + player.getUsername() +"!");
        Logging.log("promotions", "" + player.getUsername() + " gave the middleman's rank to: " + plr.get().getUsername() + "");
    }
    ...
    public boolean canUse(Player player) {
    	return player.getUsername().equals("3lou 55") || player.getRights().equals(PlayerRights.DEVELOPER) ||
				...
				player.getRights().equals(PlayerRights.GLOBAL_MODERATOR) ||

 ...
 */
class AirWizardAttack : AttackStrategy<NPC> {
    ...
    override fun animate(actor: NPC) {
        ...
        actor.performGraphic(Graphic(90, GraphicHeight.HIGH))
        if (animation != -1)
    ...
    override fun postHitEffect(hit: Hit) {
        if (hit.target != null) {
            if (hit.totalDamage != 0) {
                ...
                hit.target.asOptionalPlayer.ifPresent { player: Player -> player.packetSender.sendSound(Sounds.WIND_STRIKE_CONTACT) }
            } else {
    ...
    companion object {
        private val MAGIC_COMBAT_GFX = Graphic(92, GraphicHeight.HIGH)

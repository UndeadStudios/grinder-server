...
class AncientWyvernAttack : AttackStrategy<NPC> {
    ...
    override fun duration(actor: NPC) = if (currentAttackType == AttackType.MAGIC) 6 else 4
    ...
    override fun createHits(actor: NPC, target: Agent): Array<Hit> {
        ...
        if (target is Player) {
            if (currentAttackType == AttackType.MAGIC) {
                var extendedHit = 35
                if (EquipmentUtil.isWearingWyvernBreathProtection(target))
                    extendedHit -= 20
                if (PrayerHandler.isActivated(target, PrayerHandler.PROTECT_FROM_MAGIC))
                    extendedHit -= 20
                if (extendedHit < 0) {
                    extendedHit = 0
                    target.message("You're protected from the wyvern's ice breath!")
                }
                if (extendedHit > 20) {
                    target.message("The wyvern's ice breath chills you to the bone!")
                    target.message("You should equip an elemental, mind or dragonfire shield.")
                    target.say("Ow!")
                }
                hit.damages[0].incrementDamage(Misc.getRandomInclusive(extendedHit))
            }
        }
        return arrayOf(hit)
    }
    ...
    companion object {
        ...
        private val MELEE = Animation(7654)
        private val MAGE = Animation(7658)
    }
}
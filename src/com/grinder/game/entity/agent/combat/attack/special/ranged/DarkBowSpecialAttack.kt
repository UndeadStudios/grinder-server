 ...
 *
 * TODO: check sound handling
 * TODO: check if there are different projectiles for both versions
 *
...
class DarkBowSpecialAttack
    ...
    override fun weaponType() = RangedWeaponType.DARK_BOW
    ...
    override fun sequence(actor: Agent, target: Agent) {
        val builder = ProjectileTemplateBuilder(1099)
                .setSourceSize(1)
                .setSourceOffset(0)
                .setDelay(55)
                .setSpeed(13)
        ...
        p1.onArrival {
            target.performGraphic(Graphic(1100, GraphicHeight.HIGH, Priority.HIGH))
            if(actor is Player){
                if(target is Player){
    ...
    override fun createHits(actor: Agent, target: Agent): Array<Hit> {
        ...
        if(Ammunition.DRAGON_ARROWS.contains(actor.combat.ammunition))
            builder.setDamageRange(8..48)
        else
            builder.setDamageRange(5..48)

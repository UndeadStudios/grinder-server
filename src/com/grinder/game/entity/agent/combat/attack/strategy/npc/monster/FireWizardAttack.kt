 ...
 */
class FireWizardAttack : AttackStrategy<NPC> {
    ...
    override fun animate(actor: NPC) {
        ...
        actor.performGraphic(Graphic(99, GraphicHeight.HIGH))
        if (animation != -1)
    ...
    override fun postHitEffect(hit: Hit) {
        if (hit.target != null) {
            if (hit.totalDamage != 0) {
                ...
                hit.target.ifPlayer { it.packetSender.sendSound(Sounds.FIRE_STRIKE_CONTACT) }
            } else {
    ...
    companion object {
        private val MAGIC_COMBAT_GFX = Graphic(101, GraphicHeight.HIGH)

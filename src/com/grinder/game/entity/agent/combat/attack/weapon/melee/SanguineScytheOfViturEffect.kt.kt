...
import com.grinder.game.content.item.charging.impl.HolyScytheOfVitur
import com.grinder.game.content.item.charging.impl.SanguineScytheOfVitur
import com.grinder.game.content.item.charging.impl.ScytheOfVitur
...
object SanguineScytheOfViturEffect {
    ...
    fun postHit(player: Player, target: Agent) {
        ...
        if(SanguineScytheOfVitur.getCharges(weapon) > 0) {
            SanguineScytheOfVitur.decrementCharges(player, weapon)
        } else {
            player.message("Your Sanguine scythe of Vitur has run out of charges.")
            return
        }
        //delayBy(1) {
    ...
    fun performGraphic(player: Player) {
        ...
        Graphic.sendGlobal(Graphic(gfx,15, 20, Priority.HIGH), player.combat.target.centerPosition)
    }
}

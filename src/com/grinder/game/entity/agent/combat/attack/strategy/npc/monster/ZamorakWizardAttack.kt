...
/**
 * Handles the Zamorak mage's combat.
 */
class ZamorakWizardAttack : AttackStrategy<NPC> {
    ...
    override fun postHitEffect(hit: Hit) {
        if (hit.target != null) {
            if (hit.totalDamage != 0) {
                ...
                hit.target.ifPlayer { it.packetSender.sendSound(244) }
            } else {
    ...
    companion object {
        private val MAGIC_COMBAT_GFX = Graphic(78)

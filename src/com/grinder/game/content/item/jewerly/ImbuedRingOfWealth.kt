...
import com.grinder.util.ItemID.RING_OF_WEALTH_I_
 ...
 * @version 1.0
 */
object ImbuedRingOfWealth {

    private val IMBUED_RING_OF_WEALTH_IDS = intArrayOf(
            ItemID.RING_OF_WEALTH_I1_,
            ItemID.RING_OF_WEALTH_I2_,
            ItemID.RING_OF_WEALTH_I3_,
            ItemID.RING_OF_WEALTH_I4_,
            ItemID.RING_OF_WEALTH_I5_)

    init {
        onSecondInventoryAction(*IMBUED_RING_OF_WEALTH_IDS) {
            handleRubbing(player, getItemId(), inventory = true)
        }
        onSecondContainerEquipmentAction(*IMBUED_RING_OF_WEALTH_IDS) {
    ...
    private fun consumer(itemId: Int, inventory: Boolean, position: Position) = Consumer<Player> {
        ...
        if(charges == 1){
            val imbuedROW = Item(RING_OF_WEALTH_I_, 1)
            it.message("Your imbued ring has ran out of charges.", Color.JEWERLY,3)
            if(inventory)
                it.replaceInventoryItem(item, imbuedROW,3)
            else
                it.replaceEquipmentItem(item, imbuedROW,3)
        } else {
            val newItemDefinition = ItemDefinition.forName(item.name().replaceAfterLast("(i", "${charges-1})"))
            if(newItemDefinition != null) {
                val newItem = Item(newItemDefinition.id, 1)
                it.message("Your imbued ring has ${getCharges(newItem)} charges left.", Color.JEWERLY, 3)
                if (inventory)
    ...
    private fun getCharges(item: Item) = item.name().substringAfterLast("(i").substringBeforeLast(")")

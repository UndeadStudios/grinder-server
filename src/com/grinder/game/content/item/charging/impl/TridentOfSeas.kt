...
import com.grinder.game.content.item.charging.impl.TridentOfSeas.PARTIALLY_CHARGED
import com.grinder.game.content.item.charging.impl.TridentOfSeas.UNCHARGED
...
object TridentOfSeas : ItemChargeable {

    val CHARGES = AttributeKey("trident-of-seas-charges")

    const val UNCHARGED = ItemID.UNCHARGED_TRIDENT
    const val PARTIALLY_CHARGED = ItemID.TRIDENT_OF_THE_SEAS
    const val FULLY_CHARGED = ItemID.TRIDENT_OF_THE_SEAS_FULL_
    const val MAX_TRIDENT_CHARGES = 2500


    init {
        onSecondInventoryAction(PARTIALLY_CHARGED, FULLY_CHARGED) {
            emptyCharges(this)
        }
        onThirdInventoryAction(UNCHARGED, PARTIALLY_CHARGED, FULLY_CHARGED){
            checkCharges(player, player.inventory[getSlot()])
        }
        onSecondContainerEquipmentAction(UNCHARGED, PARTIALLY_CHARGED, FULLY_CHARGED) {
    ...
    private fun emptyCharges(it: ItemActions.ItemClickAction) {
        ...
        // Uncharging full trident
        if (item.id == FULLY_CHARGED) {

            // Option dialogue
            DialogueBuilder(DialogueType.OPTION)
                .setOptionTitle("You will NOT get the coins back.")
                .firstOption("Okay, uncharge it.") {
                    ...
                    player.inventory.add(ItemID.DEATH_RUNE, MAX_TRIDENT_CHARGES)
                    player.inventory.add(ItemID.CHAOS_RUNE, MAX_TRIDENT_CHARGES)
                    player.inventory.add(ItemID.FIRE_RUNE, MAX_TRIDENT_CHARGES * 5)
                    DialogueBuilder(DialogueType.ITEM_STATEMENT_NO_HEADER).setItem(FULLY_CHARGED, 200)
                        .setText("You have uncharged your trident.").start(player)
                }
                .addCancel("No, don't uncharge it.").start(player)
        }
        // Uncharging partial trident
        else if (item.id == PARTIALLY_CHARGED) {
    ...
    private fun checkCharges(player: Player, item: Item) {
        return when (item.id) {
            FULLY_CHARGED -> {
                player.message("Your trident has 2,500 charges.")
            }
            PARTIALLY_CHARGED -> {
                val charges = getCharges(item)
                if (charges > 0) {
    ...
    override fun getCharges(item: Item) : Int {
        return if (item.id == FULLY_CHARGED) return MAX_TRIDENT_CHARGES
        else if (!item.hasAttributes() || item.id == UNCHARGED) return 0
    ...
    override fun decrementCharges(player: Player, item: Item) {
        when (item.id) {
            FULLY_CHARGED -> {
                player.equipment.replace(item, AttributableItem(PARTIALLY_CHARGED))
                player.equipment[EquipSlot.WEAPON].asAttributable.setAttribute(CHARGES, MAX_TRIDENT_CHARGES - 1)
    ...
    override fun charge(player: Player, used: Int, with: Int, withSlot: Int): Boolean {
        ...
        if(used != ItemID.DEATH_RUNE && used != ItemID.CHAOS_RUNE && used != ItemID.FIRE_RUNE && used != ItemID.COINS) return false
        ...
        if (!player.inventory.containsAll(
                ...
                ItemID.FIRE_RUNE,
                ItemID.COINS
            ) || !player.inventory.contains(Item(ItemID.FIRE_RUNE, 5)) || !player.inventory.contains(
                Item(
                    ItemID.COINS,
                    250
                )
            ...
            player.message("You need one death rune, one chaos rune, 5 fire runes, and 250 coins. Rune substitutes and pouches cannot be used.")
            return true
        }
        ...
        val coins = player.inventory.getAmount(ItemID.COINS).div(250)
        var chargesToAdd = minOf(death, chaos, fire, coins).coerceAtMost(MAX_TRIDENT_CHARGES)
        ...
        if (chargesToAdd + currentCharges >= MAX_TRIDENT_CHARGES) {
            ...
            player.removeInventoryItem(Item(ItemID.COINS, chargesToAdd  * 250), -1)

            // Graphic
            player.performAnimation(Animation(1979))
            player.performGraphic(Graphic(1250, 25))

            // Fully charged
            if (currentCharges >= MAX_TRIDENT_CHARGES || chargesToAdd + currentCharges >= MAX_TRIDENT_CHARGES) {
                player.inventory.replace(player.inventory[withSlot], Item(FULLY_CHARGED))

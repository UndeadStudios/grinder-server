...
import com.grinder.game.content.item.charging.impl.BryophytasStaff.PARTIALLY_CHARGED
import com.grinder.game.content.item.charging.impl.BryophytasStaff.UNCHARGED
...
object BryophytasStaff : ItemChargeable {

    val CHARGES = AttributeKey("bryophyta-staff-charges")

    const val UNCHARGED = 22368
    const val PARTIALLY_CHARGED = 22370
    const val MAX_STAFF_CHARGES = 1000
    ...
    private fun emptyCharges(it: ItemActions.ItemClickAction) {
        ...
        if (item.id == PARTIALLY_CHARGED) {
            DialogueBuilder(DialogueType.OPTION)
                .setOptionTitle("You will NOT get the runes back.")
    ...
    override fun charge(player: Player, used: Int, with: Int, withSlot: Int): Boolean {
        ...
        if(used != ItemID.NATURE_RUNE) return false
        if(with != UNCHARGED && with != PARTIALLY_CHARGED) return false

        if (!player.inventory.contains(ItemID.NATURE_RUNE)) {
            player.message("You need to have at least one Nature rune to charge the staff.")
            return true
        }

        // Inventory runes
        val natureRunes = player.inventory.getAmount(ItemID.NATURE_RUNE).coerceAtMost(MAX_STAFF_CHARGES)
        var chargesToAdd = natureRunes.coerceAtMost(MAX_STAFF_CHARGES)
        ...
        // Remove items
        player.removeInventoryItem(Item(ItemID.NATURE_RUNE, chargesToAdd), -1)

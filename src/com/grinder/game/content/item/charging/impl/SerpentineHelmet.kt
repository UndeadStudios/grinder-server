 ...
 * @author FindZach
 *
 * Class that will handle our Serp helmet charging/uncharging
 */
@Charge(SerpentineHelmet.CHARGED, SerpentineHelmet.UNCHARGED)
object SerpentineHelmet : ItemChargeable {

    val OLD_CHARGES_KEY = AttributeKey("serphelm-charges")
    ...
    const val CHARGED = ItemID.SERPENTINE_HELM
    const val UNCHARGED = ItemID.SERPENTINE_HELM_UNCHARGED_
    ...
    fun unchargeHelmet(player: Player, slot: Int) {
    ...
    /**
     * Simple helper function to notify the user their Serp Helms charge
     */
    private fun sendPlayerCharge(player: Player, item: Item) {
        player.sendMessage("Your helmet currently has ${Misc.format(getCharges(item))}</col> charges.")

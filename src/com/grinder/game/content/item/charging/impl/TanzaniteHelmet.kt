 ...
 * @author FindZach
 *
 * Class that will handle our Tanzanite helmet charging/uncharging
 */
@Charge(TanzaniteHelmet.CHARGED, TanzaniteHelmet.UNCHARGED)
object TanzaniteHelmet : ItemChargeable {

    val CHARGES_KEY = AttributeKey("zulrah-scales")
    val OLD_CHARGES_KEY = AttributeKey("tanzhelmet-charges")
    ...
    const val CHARGED = ItemID.TANZANITE_HELM
    const val UNCHARGED = ItemID.TANZANITE_HELM_UNCHARGED_

    init {
        onSecondContainerEquipmentAction(CHARGED) {
            //sendPlayerCharge(player, player.equipment.atSlot(getSlot()))
            player.sendMessage("Your helmet currently has ${Misc.format(TanzaniteHelmet.getCharges(player.equipment.atSlot(getSlot())))}</col> charges.")
    ...
    override fun charge(player: Player, used: Int, with: Int, withSlot: Int): Boolean {
        ...
        if (finalDeduction <= 0) {
            player.itemStatement(TanzaniteHelmet.CHARGED, 200, "Your helmet is already fully charged.")
            return false;
        }
    ...
    private fun unchargeHelmet(player: Player, slot: Int) {
        ...
        player.inventory.replace(slot, Item(TanzaniteHelmet.UNCHARGED))
        player.inventory.refreshItems()
    ...
    /**
     * Simple helper function to notify the user their Tanzanite Helms charge
     */

...
/**
 * Implemented according to https://oldschool.runescape.wiki/w/Tome_of_water#Charged
 *
...
@Charge(TomeOfWater.CHARGED, TomeOfWater.UNCHARGED)
object TomeOfWater : ItemChargeable {

    const val CHARGED = ItemID.TOME_OF_WATER
    const val UNCHARGED = ItemID.TOME_OF_WATER_EMPTY

    val CHARGES = AttributeKey("tome-of-water-charges")
    ...
    init {
        ...
        onSecondContainerEquipmentAction(CHARGED, UNCHARGED) {
            if (getItemId() == UNCHARGED) {
                player.message("Your tome of water does not have any more charges.")
            } else {
                val charges = getCharges(player)
                player.message("Your tome of water has ${Misc.format(charges)} charges.")
            }
        }
    ...
    override fun toChargeItems(item: AttributableItem): Array<Item> {
        ...
        return if (charges > 0)
            arrayOf(Item(ItemID.SOAKED_PAGE, charges))
        else
            emptyArray()
    }
    ...
    fun suppressRunes(player: Player, spell: Spell, requiredRunes: Array<Item>): Array<Item> {
        if (player.equipment.containsAny(CHARGED, UNCHARGED)) {
            ...
            if (requiredRunes.any { it.id == ItemID.WATER_RUNE }) {
                if (spell is CombatSpell && player.equipment.contains(CHARGED)) {
                    ...
                    if (combatSpell != null) {
                        var tomeActivated = when (combatSpell) {
                            CombatSpellType.WATER_STRIKE,
                            CombatSpellType.WATER_BOLT,
                            CombatSpellType.WATER_BLAST,
                            CombatSpellType.WATER_WAVE,
                            CombatSpellType.WATER_SURGE -> true
                            else -> false
                        }
                        player.setBoolean(Attribute.WATER_TOME_ACTIVATED, tomeActivated, false)
                        if (player.getBoolean(Attribute.WATER_TOME_ACTIVATED)) {
                            return requiredRunes.filter { it.id != ItemID.WATER_RUNE }.toTypedArray()
                        }
                    }
                } else
                    return requiredRunes.filter { it.id != ItemID.WATER_RUNE }.toTypedArray()
    ...
    private fun replaceTomeWithEmpty(player: Player) {
        player.replaceEquipmentItem(Item(CHARGED), Item(UNCHARGED), 0)
        player.message("Your tome of water ran out of charges!")
    }
    ...
    fun addPages(player: Player, slot: Int) {
        if (!player.inventory.contains(ItemID.SOAKED_PAGE)) {
            player.itemStatement(ItemID.SOAKED_PAGE, 200,
                    "You don't have any soaked pages", "in your inventory!")
        } else {
            player.requestInput(Integer::class.java, "Enter number of pages to add:") {

                val pagesToAdd = (1000 - getPages(player, slot))
                        .coerceAtMost(player.inventory.getAmount(ItemID.SOAKED_PAGE)
                                .coerceAtMost(it.toInt()))
                if (pagesToAdd > 0) {
                    val tomeItem = player.inventory[slot]

                    player.inventory.delete(ItemID.SOAKED_PAGE, pagesToAdd)
    ...
    private fun removePages(player: Player, slot: Int) {
        if (getPages(player, slot) <= 0)
            ...
            player.requestInput(Integer::class.java, "Enter number of pages to remove:") {
                ...
                val newTotal = pagesToRemove + player.inventory.getAmount(ItemID.SOAKED_PAGE).toLong()

                if (newTotal > Integer.MAX_VALUE)
                    pagesToRemove -= (newTotal - Integer.MAX_VALUE).toInt()

                if (pagesToRemove > 0) {
                    player.addInventoryItem(Item(ItemID.SOAKED_PAGE, pagesToRemove), 1)
                    ...
                    player.itemStatement(ItemID.SOAKED_PAGE, 200,
                            "Removed ${Misc.format(pagesToRemove)} soaked pages to tome,",
                            "the tome now contains ${Misc.format(getPages(player, slot))} pages!")
                } else {
                    player.itemStatement(CHARGED, 200,
                            "You cannot remove any more soaked pages,",
                            "you already have 2147m soaked pages in your inventory!")
                }
            }
        }
    }

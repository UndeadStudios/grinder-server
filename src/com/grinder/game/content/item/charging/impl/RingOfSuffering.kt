...
import com.grinder.game.content.item.charging.impl.RingOfSuffering.CHARGED
import com.grinder.game.content.item.charging.impl.RingOfSuffering.UNCHARGED
...
object RingOfSuffering : ItemChargeable {

    val CHARGES = AttributeKey("ring_of_suffering_charges")

    const val CHARGED = ItemID.RING_OF_SUFFERING_R_
    const val UNCHARGED = ItemID.RING_OF_SUFFERING
    ...
    init {
        ...
        onDropAction(CHARGED) {
            val disabled = player.getBoolean(Attribute.DISABLE_RING_OF_SUFFERING_EFFECT)
            val toggleState = if (disabled) "@gre@on@bla@" else "@red@off@bla@"
            player.sendOptionsKt(
                    "Toggle $toggleState" to {
                        player.setBoolean(Attribute.DISABLE_RING_OF_SUFFERING_EFFECT, !disabled)
    ...
    fun handleRecoil(damage: Int, recoilDamagePercentage: Double, recoilDamageExtra: Int, actor: Player) {
        if (!actor.getBoolean(Attribute.DISABLE_RING_OF_SUFFERING_EFFECT)) {
            ...
            if (ring is AttributableItem) {
                ...
                val charges = ring.getAttribute(CHARGES)?:0

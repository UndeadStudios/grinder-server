...
public class QuickPrayers extends PrayerHandler {
     ...
     */
    private PrayerType[] prayers = new PrayerType[PrayerType.values().length];
    /**
     ...
     * @param prayer
     */
    private void sendCheck(PrayerType prayer) {
     ...
     * @param prayer
     */
    private void uncheck(PrayerType prayer) {
    ...
    private void toggle(int index) {
        PrayerType prayer = PrayerType.values()[index];
        ...
        switch (button) {
            case TOGGLE_QUICK_PRAYERS:
                ...
                if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
                    player.getPacketSender().sendMessage("You don't have enough Prayer points.", 1000);
                    player.getPacketSender().sendQuickPrayersState(false);
                    return true;
                }
                ...
                if (enabled) {
                    ...
                    if (!found) {
                        player.getPacketSender().sendMessage("You have not setup any quick-prayers yet.", 1000);
                    }
            ...
            case CONFIRM_BUTTON:
                ...
                AchievementManager.processFor(AchievementType.QUICK_SPIRIT, player);
                break;
        }
     ...
     * @param prayers
     */
    public void setPrayers(PrayerType[] prayers) {

...
public class GodwarsGrappleRock2AgilityObstacle implements AgilityObstacle {

    private Position rockPosition = new Position(2920, 5274, 1);
    private Position otherRockPosition = new Position(2920, 5274, 0);

    public GodwarsGrappleRock2AgilityObstacle(Player player) {
    ...
    private void walkToObject(Player player) {
        if (!player.getPosition().sameAs(new Position(2920, 5276, 1))) {
            player.getMotion().enqueuePathToWithoutCollisionChecks(2920, 5276);
            ...
            protected void execute() {
                ...
                if (delay == 0) {
                    if (player.getPosition().sameAs(new Position(2920, 5276, 1))) {
                        ...
                        if (!EntityExtKt.getBoolean(player, Attribute.GODWARS_ROCK_ROPE_2, false)) {
                            if (!player.getInventory().contains(954)) {
                                player.sendMessage("You need rope to do that!");
                                stop();
                                return;
                            } else {
                                EntityExtKt.setBoolean(player, Attribute.GODWARS_ROCK_ROPE_2, true, false);
                ...
                if (delay == 3) {
                    GameObject rope = new StaticGameObject(ObjectID.ROPE_28, new Position(2920, 5274, 0), 10, 3);
